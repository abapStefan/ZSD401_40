...
CLASS lcl_passenger_flight DEFINITION .
  ...
  PRIVATE SECTION.
    TYPES: BEGIN OF st_flights_buffer,
             carrier_id     TYPE /lrn/passflight-carrier_id,                                  " Hier die 2 Änderung
             ...
             seats_free     TYPE i,                                                           " Übung 11_1
             price          TYPE /lrn/passflight-price,
             currency_code  TYPE /lrn/passflight-currency_code,
           END OF st_flights_buffer.
*    CLASS-DATA: flights_buffer TYPE TABLE OF st_flights_buffer.                              " Übung 16_3

*      CLASS-DATA: flights_buffer TYPE SORTED TABLE OF st_flights_buffer                      " Übung 17_1
*                                 WITH NON-UNIQUE KEY carrier_id connection_id flight_date.   " Übung 16_4

      CLASS-DATA: flights_buffer TYPE HASHED TABLE OF st_flights_buffer
                                 WITH UNIQUE KEY carrier_id connection_id flight_date           " Übung 17_2
    ...
    TYPES:
      BEGIN OF st_connections_buffer,                                    " Übung 05 / 01
        ...
        timezone_to     TYPE timezone,                                   " Übung 10_2
        duration        TYPE i,                                          " Ubung 07 / 2
      END OF st_connections_buffer.

*    CLASS-DATA connections_buffer TYPE TABLE OF st_connections_buffer.    " Übung 05 / 02  " Übung 16_1


     CLASS-DATA connections_buffer TYPE HASHED TABLE OF st_connections_buffer               " Übung 16_2
...
CLASS lcl_passenger_flight IMPLEMENTATION.
  ...
  METHOD class_constructor.                                               " Übung 05 / 03

*     SELECT                                                              " Übung 7  Übung 10_4
*        FROM /lrn/airport
...
*         INTO TABLE @DATA(airports).
     ...
     SELECT FROM /dmo/connection AS c                                     " Übung 10_1
*          RIGHT OUTER JOIN  /lrn/airport AS f      " Right outer Join ergibt Fehler ITAB LINE NOT FOUND
...
*      DATA(today) = cl_abap_context_info=>get_system_date(  ).                           " Übung 07 / 4


** Übung 12_2 diesen Loop Kommentieren

*      LOOP AT connections_buffer INTO DATA(connection).                                  " Übung 07 / 3
*
*      CONVERT DATE today                                                                 " Übung 07 / 5
*              TIME connection-departure_time
**              TIME ZONE airports[ airport_id = connection-airport_from_id ]-timzone
*              TIME ZONE connection-timezone_from                                             " Übung 10_3
...
*               TIME ZONE connection-timezone_to                                             " Übung 10_3
...
*       ENDLOOP.
  ...
  METHOD get_flights_by_carrier.
   ...
   IF NOT line_exists( flights_buffer[  KEY sk_carrier COMPONENTS
                                            carrier_id = i_carrier_id ] ).              " Übung 14_3

    SELECT
      FROM /lrn/passflight
    FIELDS carrier_id, connection_id, flight_date,                                " Hier alle Felder aufnehmen
           plane_type_id, seats_max, seats_occupied,
           seats_max - seats_occupied AS seats_free,                              " Übung 11_2
*           price, currency_code                                                  " Übung 12_3
           currency_conversion(
             amount            = price,
     ...
     ORDER BY flight_date ASCENDING                                               " Übung 11_1
*      INTO TABLE @flights_buffer.                                                 "  Hier die 1 änderung in Tabelle
     APPENDING TABLE @flights_buffer.                                             " Übung 14_2

*     SORT flights_buffer BY carrier_id connection_id flight_date.                 " Übung 14_3   Übung 16_5
*     DELETE ADJACENT DUPLICATES FROM flights_buffer                              " Übung 14_3
...
*     r_result = VALUE #(                                                              " Übung 14 A_3
*                  FOR flight IN flights_buffer

      r_result = VALUE #(                                                             " Übung 15_1
                   FOR <flight> IN flights_buffer

                   USINg KEY sk_carrier                                                " Übung 17_4

                WHERE ( carrier_id = i_carrier_id )

*                   ( NEW lcl_passenger_flight( i_carrier_id   = flight-carrier_id
*                                               i_connection_id = flight-connection_id
*                                               i_flight_date   = flight-flight_date ) ) ).

                    ( NEW lcl_passenger_flight( i_carrier_id   = <flight>-carrier_id        " Übung 15_2
                                               i_connection_id = <flight>-connection_id
                                               i_flight_date   = <flight>-flight_date ) ) ).
    ...
    TRY.
      ...
      CATCH cx_sy_itab_line_not_found.
        ...
        FIELDS plane_type_id,
               ...
               seats_max - seats_occupied AS seats_free,                  " Übung 11_3
*               price, currency_code
            currency_conversion(
                amount            = price,
                source_currency  = currency_code,
...
*      seats_free = flight_raw-seats_max - flight_raw-seats_occupied.
      seats_free = flight_raw-seats_free.                                  " Übung 11_4
      price      = flight_raw-price.                                       " Übung 12 WICHTIG sonst null

** convert currencies                                                      " Übung 12_4
*      TRY.
...
*        INTO @connection_details .                                                          " Übung 05 / 04
...
*    DATA carrier_id    TYPE /dmo/connection    READ-ONLY.                            " Übung 6 / 10 falsch
*    DATA connection_id TYPE /dmo/carrier_id       READ-ONLY.


    DATA carrier_id    TYPE /dmo/carrier_id    READ-ONLY.                             " Übung 6 /10 Richtig
    DATA connection_id TYPE /dmo/connection_id       READ-ONLY.
    DATA flight_date   TYPE /dmo/flight_date      READ-ONLY.
...
*    carrier_id = EXACT #( i_carrier_id ).                                                  " Übung 6 / 8 oder 9
...
*    TYPES t_output TYPE c LENGTH 40.
    ...
    TYPES t_output TYPE  string.                                                          " Übung 6 / 4 1 Teil
...
*    SELECT SINGLE                                                       " Übung 18_1 alles Kommentieren
*      FROM /lrn/carrier
**    FIELDS name, currency_code                                         " Übung 11_6
*     FIELDS concat_with_space( carrier_id, name, 1 ), currency_code
*     WHERE carrier_id = @i_carrier_id
*     INTO ( @me->name, @me->currency_code ).

*      SELECT SINGLE                                                      " Übung 18_2
*      FROM /lrn/i_carrier
**    FIELDS name, currency_code                                         " Übung 11_6
...
*     INTO ( @me->name, @me->currency_code ).
     ...
     SELECT SINGLE                                                      " Übung 18_4   wieder alles neu
     FROM /lrn/carrier
*    FIELDS name, currency_code                                         " Übung 11_6
     FIELDS concat_with_space( carrier_id, name, 1 ), currency_code
     WHERE carrier_id = @i_carrier_id
    ...
    ENDIF.
     ...
     AUTHORITY-CHECK OBJECT '/LRN/CARR'                                  " Übung 18_5
...
*    name = carrier_id && ` ` && name.                                    "Übung_11_5
...
*        DATA(days_later) =  i_from_date - flight->flight_date.                            " Übung 3 / hier der Fehler
...
*    DATA total TYPE i.                                                                   " Übung 13_3 alles kommentieren
*
...
*    SELECT FROM /lrn/passflight                                                            " Übung 13_4
*        FIELDS SUM( seats_max - seats_occupied ) AS sum,                                   " Übung 14 alles kommentieren
*        COUNT(*) AS count
*        WHERE carrier_id = @carrier_id
